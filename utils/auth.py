...
def complete_2fa_flow(client, secret, login_response):
    ...
    This mirrors the Playwright implementation:
    - passport.verify2FAPageVisible()
    - passport.generate2FACode(secret)
    - passport.enter2FACode(code)
    - passport.submit2FACode()

    Args:
        client: Locust HttpSession client
        secret (str): Base32-encoded 2FA secret (shared across all load test users)
        login_response: The response from the initial login POST (may contain redirect to 2FA page)

    Returns:
        bool: True if 2FA completed successfully
    """
    ...
    if not form:
        ...
        print(f"DEBUG: Response body (1st 1000): {twofa_page_response.text[:1000]}")
        return False
